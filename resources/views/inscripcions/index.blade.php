...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Materias de Estudiantes</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">Inicio</a></li>
                        <li class="breadcrumb-item active">Materias de Estudiantes</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('inscripcions.create') }}" class="btn btn-info"><i class="fa fa-plus"></i>
                                Nuevo</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                                    ...
                                    @foreach ($inscripcions as $inscripcion)
                                        <tr>
                                            ...
                                            <td>{{ $inscripcion->estudiante->nombre }}
                                                {{ $inscripcion->estudiante->paterno }}
                                                {{ $inscripcion->estudiante->materno }}</td>
                                            <td>{{ $inscripcion->nivel }}</td>
                                            <td>{{ $inscripcion->grado }}º</td>
                                            <td>{{ $inscripcion->paralelo->paralelo }}</td>
                                            <td>{{ $inscripcion->turno }}</td>
                                            <td class="btns-opciones">
                                                <a href="{{ route('inscripcions.formulario', $inscripcion->id) }}"
                                                    target="_blank" class="ir-evaluacion" data-toggle="tooltip"
                                                    data-placement="left" title="Formulario"><i
                                                        class="fa fa-file-pdf"></i></a>

                                                @if (Auth::user()->tipo == 'ADMINISTRADOR')
                                                    <a href="{{ route('inscripcions.edit', $inscripcion->id) }}"
                                                        class="modificar"><i class="fa fa-edit" data-toggle="tooltip"
                                                            data-placement="left" title="Modificar"></i></a>

                                                    <a href="#"
                                                        data-url="{{ route('inscripcions.destroy', $inscripcion->id) }}"
